...
const inputStyle = (state: InputState): any => {
  switch (state) {
    case 'valid':
      return {
...
const showMarker = (state: InputState): JSX.Element | null => {
  switch (state) {
    case 'valid':
      return (
        <AntDesign
...
const SignupScreen: React.FC<{navigation: any}> = ({navigation}) => {
  ...
  const checkUserExist = async (type: string, value: string): Promise<void> => {
    if (value?.length > 0) {
